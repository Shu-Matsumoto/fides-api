...
{
    // 女優プロフィール画像保存
    public function actorProfileImageStore(Request $request, int $userId)
    {
        // 対象ユーザー取得
        $image_path = ActorUser::find($userId)->image_path;
        ...
        return response()->json([
            'message' => 'success',
            'data' =>  ActorUser::find($userId),
        ], 200);
    }

    // メーカープロフィール画像保存
    public function makerProfileImageStore(Request $request, int $userId)
    {
        // 対象ユーザー取得
        $image_path = MakerUser::find($userId)->image_path;
        $delete_image_path = $image_path;
        // DB上パスから先頭の"storage/"を削除し、先頭に"public/"を付加する
        if (strncmp($delete_image_path, 'storage/', 8) == 0) {
            $delete_image_path = "public/" . substr($delete_image_path, 8, strlen($delete_image_path) - 8);
